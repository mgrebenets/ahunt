...
public class XLActivity extends Activity implements AdEventsListener {
	...
	private final Runnable mResumeAfterCrash = new Runnable() {
		public void run() {
			...
			if (mGameThread.penalties >= 6) {
				...
				// also show the game start buttons
				((ImageButton)findViewById(R.id.btnGameI)).setVisibility(View.VISIBLE);
				((ImageButton)findViewById(R.id.btnGameII)).setVisibility(View.VISIBLE);		
			} else {
	...
	private void updateSound() {
		// substitute current sound button image
		if (mSoundOn) {
	...
	private OnClickListener mGameModeListener = new OnClickListener() {
		public void onClick(View v) {
			...
			// also hide both game start buttons
			((ImageButton)findViewById(R.id.btnGameI)).setVisibility(View.INVISIBLE);
			((ImageButton)findViewById(R.id.btnGameII)).setVisibility(View.INVISIBLE);
		}
	};

	private void updatePause() {
		// update current image in pause button, show/hide paused label
		if (mGameThread.state == STATE_PAUSED) {
